...
% The title, the authors and the affiliations. scripts/new_revision.py reads them
% from here when it writes a response letter, so they are only ever typed once.
...
\section*{Abstract}
% An anchor for the response letter, which writes \lnp{ln:abstract} and gets
% back "(line 2)" -- whichever line this text has drifted to by the time the
% letter is built. Add one wherever a reviewer needs to be sent, and keep the
% ln: prefix so the anchors stay recognisable among the other labels.
\linelabel{ln:abstract}%
One paragraph summarising the problem, what was done about it, and what came out.
...
\linelabel{ln:gap}%
State what remains unresolved, and why the existing approaches cannot resolve it
