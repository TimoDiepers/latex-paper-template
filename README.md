...
**Generated by `scripts/make_submission.py`**
...
`make_submission.py` always builds the package for the newest one, so neither needs
...
The package you send is unaffected either way, because `make_submission.py` builds
...
`uv run scripts/make_submission.py [stage]` works on the newest folder by default. It
