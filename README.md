...
generated from them by a script, and regenerated from scratch each time it runs, so
nothing has to be kept in sync manually.

**Maintained by you**

- `manuscript.tex` — an ordinary LaTeX manuscript; from the first revision onwards it
  is `manuscript_annotated.tex` and carries `\added{…}`, `\replaced{…}{…}` and
  `\deleted{…}` where the text changed
- `response_to_reviewers.tex` — from the first revision onwards, referring to the
...
- `response_to_reviewers.pdf` — the letter, its `(line 42)` references resolved
  against the manuscript in the same package
...
letter's line references cannot drift out of date. Internal notes — target journal,
suggested reviewers, table of contents, `%` comments — are removed from everything
that ships, and only cited references are inlined, so `references.bib` can remain a
Zotero or Citavi export and is never sent.
